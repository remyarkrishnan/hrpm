...
@section('title', 'Project Progress - ' . $project->name)
@section('page-title', 'Progress Dashboard: ' . $project->name)

@section('content')
<div class="page-header">
    <div>
        <p>Track timeline, completion %, and delays for this project.</p>
    </div>
    <div class="header-actions">
        <a href="{{ route('admin.projects.index') }}" class="btn-primary" style="margin: 10px 0">
            <i class="material-icons">arrow_back</i> Back to Projects
        </a>
    </div>
</div>
...
<div class="stats-grid">
    <div class="stat-card">
        <div class="stat-icon" style="background:#4CAF50;"><i class="material-icons">check_circle</i></div>
        <div class="stat-info">
            <h3>{{ $completion ?? 0 }}%</h3>
    ...
    <div class="stat-card">
        <div class="stat-icon" style="background:#F44336;"><i class="material-icons">warning</i></div>
        <div class="stat-info">
            <h3>{{ $delayed ?? 0 }}</h3>
    ...
    <div class="table-responsive">
        <table class="shifts-table">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted">No steps/subplans found</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
.shifts-table th, .shifts-table td { padding:12px; border-bottom:1px solid #f0f0f0; vertical-align:middle; text-align:left; }
.shifts-table th { background:#f5f5f5; font-weight:600; }
.progress { background:#f0f0f0; border-radius:6px; height:8px; margin-bottom:4px; }
.progress-bar { border-radius:6px; background:#4CAF50; height:100%; }
.status-badge { padding:4px 10px; border-radius:12px; font-weight:600; display:inline-block; font-size:12px; text-transform:none; }
...
.header-actions .btn-primary { display:inline-flex; align-items:center; gap:8px; background:#6750A4; color:white; padding:10px 16px; border-radius:8px; text-decoration:none; font-weight:500; }
...
new Chart(ctx, {
    ...
    options: {
        indexAxis: 'y',
        responsive: true,
        scales: { x: { max: 100, title:{display:true, text:'% Complete'} } },
