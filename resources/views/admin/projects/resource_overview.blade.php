...
@section('title', 'Project Resource Overview - ' . $project->name . env('COMPANY_NAME', 'Teqin Vally'))
@section('page-title', 'Project Resource Overview - ' .  $project->name )

@section('content')
<div class="page-header">
    <div>
        <p>Track how employees are allocated across each project step and subplan.</p>
    </div>
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <div class="stat-info">
            <h3>{{ $totalSteps ?? 0 }}</h3>
            <p>Total Steps Across Projects</p>
        </div>
    </div>
            ...
            <div class="card mb-3 p-3 border-light shadow-sm">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h5 class="fw-bold text-dark mb-0">
                        Step {{ $loop->iteration }}: {{ $step->step_name }}
                    </h5>
                    <span class="badge bg-info text-dark">{{ ucfirst($step->status) }}</span>
                </div>
                    ...
                    <div class="border rounded p-3 mb-3" style="background:#fafafa;">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div>
                                ...
                                <small class="text-muted">{{ $subplan->description }}</small>
                            </div>
                            <span class="badge bg-success text-white">
                                Progress: {{ $subplan->progress_percentage ?? 0 }}%
                            </span>
                        </div>
                        ...
                        <table class="shifts-table mb-0">
                            ...
                            <tbody>
                                @forelse($subplan->allocations as $alloc)
                                    ...
                                    @endphp
                                    <tr>
                                        <td>{{ $alloc->employee->name ?? 'N/A' }}</td>
                                        <td>{{ ucfirst($alloc->role ?? '-') }}</td>
                                        <td>{{ $alloc->allocation_percentage ?? 0 }}%</td>
                                        <td><span class="status-badge {{ $perfClass }}">{{ $alloc->allocation_percentage }}%</span></td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="text-muted text-center">No employees assigned</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                @empty
                    <p class="text-muted ms-3">No subplans defined for this step.</p>
                @endforelse
            </div>
        @empty
            <p class="text-muted">No steps for this project yet.</p>
        @endforelse
    </div>
...
<style>
    ...
    .shifts-table-card { background:white; padding:24px; border-radius:12px; box-shadow:0 2px 8px rgba(0,0,0,.06); }
    .shifts-table { width:100%; border-collapse:collapse; margin-top:10px; }
    .shifts-table th, .shifts-table td { padding:12px; border-bottom:1px solid #eee; text-align:left; }
