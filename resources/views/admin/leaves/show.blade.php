    ...
    <div class="leave-header">
        <div class="leave-info">
            ...
            <div class="leave-duration">
                ...
                <span>{{ $leave->from_date->format('d-m-Y') ?? '' }} to {{ $leave->to_date->format('d-m-Y') ?? '' }}</span>
        ...
        <div class="detail-card">
            ...
            <div class="info-list">
                ...
                <div class="info-item">
                    ...
                    <span>{{ round($leave->total_days) ?? '' }} days</span>
                </div>
            </div>
        </div>
        ...
        <div class="detail-card">
            ...
            <div class="info-list">
                ...
                <div class="info-item">
                    ...
                    <span>{{ $leave->assignedManager->name ?? 'N/A' }}</span>
                </div>
                @endif
                @if($leave->manager_remarks)
                <div class="info-item">
                    <strong>Manager Remarks</strong>
                    <span>{{ $leave->manager_remarks ?? 'N/A' }}</span>
                </div>
                @endif
                @if(isset($leave->approved_by_account_and_hr))
                <div class="info-item">
                    <strong>@if($leave->status == 'approved_by_account_and_hr') Approved @else Rejected @endif By:</strong>
                    <span>{{ $leave->approver->name }}</span>
                </div>
                @endif
                @if(isset($leave->approved_at))
                <div class="info-item">
                    <strong>@if($leave->status == 'approved_by_account_and_hr') Approved @else Rejected @endif Date:</strong>
...
function deleteLeave() {
    if (confirm('Are you sure you want to delete this leave request?\n\nThis action cannot be undone.')) {
        fetch(`/admin/leaves/{{ $leave->id }}/destroy`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'Content-Type': 'application/json'
