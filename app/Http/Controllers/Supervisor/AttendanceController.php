...
class AttendanceController extends Controller
{
    ...
    $attendances = $query->where('created_by',$userId)->orderBy('date', 'desc')->paginate(15);

    // Pass list of employees and projects for filters in view
    $employees = User::whereHas('roles', fn($q) => $q->where('name', 'employee'))->get();
    $projects = Project::all();

    return view('supervisor.attendance.index', compact('attendances', 'employees', 'projects', 'attendanceCounts'));
    ...
    public function create()
    {
        ...
        return view('supervisor.attendance.create', compact('employees', 'projects'));
    }
   ...
   public function bulk()
    {
        ...
        return view('supervisor.attendance.bulk', compact('employees', 'projects'));
   ...
   public function bulkStore(Request $request)
   {
    ...
    $checkInTime = Carbon::parse("$date {$validated['check_in']}:00");
    $checkOutTime = !empty($validated['check_out'])
        ? Carbon::parse("$date {$validated['check_out']}:00")
        : null;

    $workingHours = $checkOutTime
        ? $checkOutTime->diffInMinutes($checkInTime) / 60
        : 0;
    ...
    foreach ($validated['user_id'] as $userId) {
        // Avoid duplicate attendance for same date + user
    ...
    $message = "{$created} attendance record(s) created successfully.";
    if (!empty($skipped)) {
        $message .= " Skipped " . count($skipped) . " (already marked).";
    }
    ...
    public function edit($id)
    {
        ...
        return view('supervisor.attendance.edit', compact('attendance', 'projects','projectLocations'));
    }
